...
async function analyzePlan(text: string): Promise<string | null> {
  ...
  const systemPrompt = `你係一個活動計劃書分析助手。請根據以下計劃書內容，提取並整理所有關鍵資訊。

## 分析規則（非常重要）：
1. **唔可以用固定格式** — 每份計劃書格式都可能唔同，你要靈活理解內容結構
2. 提取所有可以識別到嘅資訊：活動名稱、日期、時間、地點、活動流程/時間表、每個環節嘅負責人、財務預算/收支預算、物資清單、場地佈置、人手安排、宣傳計劃、風險評估 等等
3. 用**繁體中文**輸出，保持專業用語
4. 如果某項資訊喺計劃書中搵唔到，唔好作，標示「（未註明）」
5. 保持原始事項嘅編號或排序
6. 特別注意數字（金額、日期、時間、人數）要準確提取
